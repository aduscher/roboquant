...
import org.robok.common.addNotNull
import org.robok.common.percent
...
open class FlexPolicyConfig(
    ...
    var minPrice: org.robok.common.Amount? = null,
)
...
open class FlexTrader(
    ...
    protected val logger = _root_ide_package_.org.robok.common.Logging.getLogger(FlexTrader::class)
     ...
     * This method will only be invoked if the signal is not closing a position.
     */
    open fun calcSize(amount: Double, signal: Signal, price: Double): org.robok.common.Size {
        require(config.fractions >= 0) { "fractions has to be >= 0, found ${config.fractions}" }
        val singleContractValue = signal.asset.value(_root_ide_package_.org.robok.common.Size.ONE, price).value
        val size = BigDecimal(amount / singleContractValue).setScale(config.fractions, RoundingMode.DOWN)
        return _root_ide_package_.org.robok.common.Size(size)
    }
     ...
     *
     * Overwrite this method if you want to create orders other than the default [org.robok.common.Order].
     */
    open fun createOrder(signal: Signal, size: org.robok.common.Size, priceItem: PriceItem): org.robok.common.Order? {
        return _root_ide_package_.org.robok.common.Order(signal.asset, size, priceItem.getPrice())
    }
     ...
     */
    open fun amountPerOrder(account: Account): org.robok.common.Amount {
     ...
     */
    private fun meetsMinPrice(asset: org.robok.common.Asset, price: Double, time: Instant): Boolean {
     ...
     */
    private operator fun Collection<org.robok.common.Order>.contains(asset: org.robok.common.Asset) = any { it.asset == asset }
    ...
    override fun createOrders(signals: List<Signal>, event: EventK, account: Account): List<org.robok.common.Order> {
        val instructions = mutableListOf<org.robok.common.Order>()
