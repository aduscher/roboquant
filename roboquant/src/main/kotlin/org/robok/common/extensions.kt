...
import org.robok.common.Config.EPS
 ...
 */
fun String.toCurrencyPair(): Pair<CurrencyK, CurrencyK> {
    val codes = split('_', '-', ' ', '/', '.', ':')
    return when (codes.size) {
        2 -> {
            val c1 = CurrencyK.getInstance(codes.first().uppercase())
            val c2 = CurrencyK.getInstance(codes.last().uppercase())
            Pair(c1, c2)
        }
        1 if length == 6 -> {
            // Assume we have two currency-codes, each of lengths 3
            val c1 = CurrencyK.getInstance(substring(0, 3).uppercase())
            val c2 = CurrencyK.getInstance(substring(3, 6).uppercase())
            Pair(c1, c2)
        }
        else -> {
