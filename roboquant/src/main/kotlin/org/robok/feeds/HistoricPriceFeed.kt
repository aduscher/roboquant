...
open class HistoricPriceFeed : HistoricFeed {
     ...
     */
    fun first(): EventK = EventK(events.firstKey(), events.getValue(events.firstKey()))

    /**
     * Return the last event in this feed
     */
    fun last(): EventK = EventK(events.lastKey(), events.getValue(events.lastKey()))
    ...
    override suspend fun play(channel: EventChannel) {
        events.forEach {
            val event = EventK(it.key, it.value)
            channel.send(event)
        }
    }
