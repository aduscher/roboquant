...
import org.robok.common.compareTo
import java.time.Instant

/**
 * Wrapper around a [Channel] for communicating the [events][EventK] of a [Feed].
...
class EventChannel(
    ...
    private val channel = Channel<EventK>(capacity, onBufferOverflow)
     ...
     */
    operator fun iterator(): ChannelIterator<EventK> = channel.iterator()
     ...
     * will be [closed].
     */
    suspend fun sendNotEmpty(event: EventK) {
     ...
     * @see Channel.receive
     */
    suspend fun receive(timeOutMillis: Long = -1): EventK {
