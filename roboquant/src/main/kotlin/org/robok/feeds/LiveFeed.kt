 ...
 * to act even if no other data is incoming. A heartbeat is an [empty event][EventK.empty].
 *
...
abstract class LiveFeed : Feed {
     ...
     */
    protected fun send(event: EventK): Unit = runBlocking {
        sendAsync(event)
    }
     ...
     */
    protected suspend fun sendAsync(event: EventK) {
