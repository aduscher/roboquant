...
import org.robok.strategies.CombinedStrategy
...
private class FastStrategy(private val skip: Int) : Strategy {
    ...
    override fun createSignals(event: EventK): List<Signal> {
...
private class FastFeed(nAssets: Int, val events: Int) : Feed {
    ...
    init {
        ...
        for (asset in assets) {
            val item = _root_ide_package_.org.robok.common.PriceBar(asset, data)
            items.add(item)
        }
    }
    ...
    override suspend fun play(channel: EventChannel) {
        repeat(events) {
            channel.send(EventK(start + it.millis, items))
        }
    }
...
private object Performance {
    ...
    private fun feedFilter(feed: FastFeed): Long {
        val asset = _root_ide_package_.org.robok.common.Stock("UNKNOWN")
        return measure {
