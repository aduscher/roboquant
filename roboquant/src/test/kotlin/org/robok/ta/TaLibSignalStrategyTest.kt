...
internal class TaLibSignalStrategyTest {
    ...
    @Test
    fun taCore() {
        val taLib = _root_ide_package_.org.robok.ta.TaLib()
    ...
    fun test() {
        ...
        val strategy = _root_ide_package_.org.robok.ta.TaLibStrategy(50)
        strategy.buy { price ->
    ...
    fun testTASignalStrategy() {
        ...
        val strategy = _root_ide_package_.org.robok.ta.TaLibSignalStrategy { asset, series ->
            // record("test", 1)
            when {
    ...
    private fun getPriceBarBuffer(size: Int): PriceBarSeries {
        val asset = Stock("XYZ")
        val result = _root_ide_package_.org.robok.ta.PriceBarSeries(size)
        repeat(size) {
    ...
    private fun run(s: Strategy, n: Int = 150): Map<Instant, List<Signal>> {
        ...
        var now = Instant.now()
        for (event in events) {
            val signals = s.createSignals(EventK(event.first, listOf(event.second)))
            result[now] = signals
            now += 1.seconds
        }
        return result
    ...
    @Test
    fun test2() {
        val strategy = _root_ide_package_.org.robok.ta.TaLibStrategy(2)
        strategy.buy {
            true
        }
    ...
    fun testStrategyFunction() {
        val s = _root_ide_package_.org.robok.ta.strategy {
            when {
