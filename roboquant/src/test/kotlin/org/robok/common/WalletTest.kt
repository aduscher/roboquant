...
internal class WalletTest {
    ...
    private val usd = CurrencyK.getInstance("USD")

    @Test
    fun getCurrencies() {
        var wallet = _root_ide_package_.org.robok.common.Wallet()
        assertTrue(wallet.currencies.isEmpty())
        wallet = _root_ide_package_.org.robok.common.Wallet(10.USD, 20.EUR)
        assertEquals(2, wallet.currencies.size)
        assertTrue(wallet.isMultiCurrency())

        wallet = _root_ide_package_.org.robok.common.Wallet(10.USD, 0.EUR)
    ...
    @Test
    fun empty() {
        val wallet = _root_ide_package_.org.robok.common.Wallet()
    ...
    @Test
    fun plusMinus() {
        val wallet = _root_ide_package_.org.robok.common.Wallet(10.USD, 20.EUR)
        val wallet2 = wallet + wallet - wallet
        assertEquals(wallet, wallet2)

        val amount = 10.USD
        val wallet3 = _root_ide_package_.org.robok.common.Wallet(amount) - amount
        assertTrue(wallet3.isEmpty())
    }

    @Test
    fun toStr() {
        val wallet = _root_ide_package_.org.robok.common.Wallet(10.USD, 20.101.EUR)
